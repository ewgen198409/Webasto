...
int glow_plug_pin = 10; // Пин для свечи накаливания
int burn_fan_pin = 9; // Пин для вентилятора горелки
int exhaust_temp_pin = A0; // Пин для датчика температуры выхлопа
int push_pin = A6; // Пин для кнопки
int pushup_pin = A7; //    кнопка вверх
int pushdown_pin = A2; //  кнопка вниз
// Подключения пинов
...
// ====================================================================================================================
// // Установка заполнения 50% (для 10-bit режима это 512)
// analogWrite(9, 512);  // Пин 9 (канал A)
// analogWrite(10, 512); // Пин 10 (канал B)

// // Или напрямую:
// OCR1B = 256;  // Установка заполнения ~50% для канала B (пин 10)
// =================  Для ARDUINOPROMINI (328PB) эту часть кода убрать =================================================
  // Настройка Timer1 (пины 9 и 10) на 500 Гц
  TCCR1A = 0;       // Сброс регистра
  TCCR1B = 0;       // Сброс регистра
  TCNT1 = 0;        // Сброс счетчика

  // Режим Fast PWM (8-bit или 10-bit)
  TCCR1A |= (1 << WGM11) | (1 << WGM10);  // 10-bit режим (больше градаций)
  TCCR1B |= (1 << WGM12);                 // Fast PWM, верхнее значение = OCR1A

  // Предделитель = 64 (для 16 МГц: 16MHz / 64 / 500Hz = 500 отсчетов)
  TCCR1B |= (1 << CS11) | (1 << CS10);    // Предделитель 64

  // Включение ШИМ на каналах A (пин 9) и B (пин 10)
  TCCR1A |= (1 << COM1A1) | (1 << COM1B1); // Неинверсный ШИМ

  // Установка верхнего предела (для точной частоты)
  // OCR1A = (F_CPU / (предделитель * частота_ШИМ)) - 1
  // Для 500 Гц: (16,000,000 / (64 * 500)) - 1 = 499
  OCR1A = 499;  // Устанавливаем верхний предел для точной частоты
// =================================================================================================================+++++
